...
import apisurvey from '@/controllers/survey'
  ...
  SURVEY_DATA(state, payload) {
    ...
    Object.keys(data).forEach((a) => {
      Object.values(data).forEach((b) => {
        if (parseInt(a) == b.form.id) {
          let data_template = b.form_template.slice(-1)[0];
          let status = b.survey.slice(-1)[0].status;
          let progress = null;
          if (data_template !== null) {
            progress = {};
            progress.progress = data_template.form_responses.progress;
            progress.status = status;
          }
          b.form_template = progress;
        }
      });
    })
    state.surveys = data
